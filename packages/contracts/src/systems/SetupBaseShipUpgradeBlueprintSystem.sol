...
uint256 constant ID = uint256(keccak256("system.SetupBaseShipUpgradeBlueprint"));

contract SetupBaseShipUpgradeBlueprintSystem is System {
  struct Args {
  ...
  function execute(bytes memory arguments) public returns (bytes memory) {
    ...
    for (uint256 i = 0; i < args.blueprints.length; ++i) {
      ...
      bt.set(id, uint32(BlueprintType.SHIP_UPGRADE));
    }
  }
