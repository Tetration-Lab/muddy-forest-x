...
if (!privateKey) {
  privateKey =
    localStorage.getItem("burnerWallet") || Wallet.createRandom().privateKey;
...
export const config: SetupContractConfig = {
  ...
  provider: {
    ...
    chainId:
      Number(params.get("chainId")) || Number(import.meta.env.VITE_CHAIN_ID),
  },
  privateKey,
  chainId:
    Number(params.get("chainId")) || Number(import.meta.env.VITE_CHAIN_ID),
  ...
  initialBlockNumber:
    ...
    Number(import.meta.env.VITE_INITIAL_BLOCK_NUMBER),
